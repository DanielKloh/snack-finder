<div class="h-full flex flex-col" x-data="{
    ...
    <flux:main container="">
        <div class="flex flex-col h-full max-w-4xl mx-auto">
            {{-- Header --}}
            <div class="mb-6">
                <flux:heading size="xl">Meal Finder Chat</flux:heading>
                <flux:text class="mt-2">Pergunte sobre refeições e receba recomendações!</flux:text>
            </div>
                                ...
                                <div class="max-w-[85%] space-y-4">
                                    ...
                                    @if(!empty($msg['meals']))
                                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            @foreach($msg['meals'] as $meal)
                                                <a href="{{ $meal['url'] }}" target="_blank" class="block">
                                                    <div class="bg-white dark:bg-neutral-800 rounded-lg p-4 shadow-sm border border-neutral-200 dark:border-neutral-700 hover:shadow-md transition-shadow">
                                                        <div class="aspect-square overflow-hidden rounded-md mb-3 bg-neutral-100 dark:bg-neutral-900 flex items-center justify-center">
                                                            <img src="http://localhost/storage/{{ $meal['imagem'] }}" alt="{{ $meal['nome'] }}" class="w-full h-full object-cover"/>
                                                        </div>
                                                        <h3 class="font-semibold text-sm text-neutral-900 dark:text-neutral-100 mb-2">{{ $meal['nome'] }}</h3>
                                                        <div class="flex items-center justify-between">
                                                            <span class="text-lg font-bold text-green-600 dark:text-green-500">{{ $meal['preco'] }}</span>
