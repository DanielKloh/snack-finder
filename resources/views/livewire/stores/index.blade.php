<div>
    <flux:main container="">
        ...
        <x-section>
            <x-table>
                <x-table.columns>
                    <x-table.column wire:click="sort('name')" sortable :sorted="$sortBy === 'name'" :direction="$sortDirection">
                        Nome
                    </x-table.column>
                ...
                <x-table.rows>
                    @foreach ($stores as $store)
                        <x-table.row wire:key="{{ $store->id }}">
                            ...
                            <x-table.cell>
                                @can('update', $store)
                                    <flux:button :href="route('stores.update', $store->id)" variant="ghost" size="sm"
                                        icon="pencil" class="cursor-pointer" inset="top bottom" wire:navigate>
                                    </flux:button>
                                @endcan
                                @can('delete', $store)
                                    <flux:button wire:click="remove({{ $store->id }})"
                                        wire:confirm="Tem certeza que deseja remover esta loja?" variant="ghost"
